...
function MyCarousel(props) {
  ...
  const links = props.links
    ? props.links.filter(emptyStringFilter) : null; // Links may not always be passed

  let captionElements;
  if (links) {
    captionElements = captions.map((c, i) => <p className="bodyFont"><Link to={links[i]}>{c}</Link></p>);
  } else {
    captionElements = captions.map((c) => <p className="bodyFont">{`${c}​`}</p>); // There's an invisible zero-width character here to prevent the caption box from changing height
  }
